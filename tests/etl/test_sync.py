...
def test_google_sheet_sync_calls_in_correct_order(tmp_path):
    """ensure_source_tabs_exist, run_sqlmesh_plan, then run_dashboard."""
    config = make_config_dict(update_type='s3')
    config['path'] = tmp_path / 'config.yml'

    call_order = []

    with patch('src.etl.ensure_source_tabs_exist') as mock_ensure:
        mock_ensure.side_effect = lambda x: call_order.append('ensure_tabs')
    ...
    assert call_order == ['ensure_tabs', 'sqlmesh', 'dashboard']


def test_google_sheet_sync_passes_config_to_all_functions(tmp_path):
    """Config dict is passed to ensure_source_tabs_exist, run_sqlmesh_plan, and run_dashboard."""
    config = make_config_dict(update_type='s3')
    config['path'] = tmp_path / 'config.yml'
...
def test_google_sheet_sync_propagates_sqlmesh_exception(tmp_path):
    ...
    with patch('src.etl.ensure_source_tabs_exist'):
        with patch('src.etl.run_sqlmesh_plan') as mock_sqlmesh:
